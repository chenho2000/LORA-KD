...
def main():
    ...
    peft_config = LoraConfig(
        r=8, lora_alpha=16, lora_dropout=0.1, task_type=TaskType.SEQ_CLS
    )
    ...
    if args.do_eval:
        ...
        student_model.eval()
        result = do_eval(student_model.base_model, task_name, eval_dataloader,
                         device, output_mode, eval_labels, num_labels)
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
        ...
        for epoch_ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                student_logits, student_atts, student_reps = student_model.base_model(input_ids, segment_ids,
                                                                                      input_mask,
                                                                                      is_student=True)
                ...
                if (global_step + 1) % args.eval_step == 0:
                    ...
                    if save_model:
                        ...
                        model_save = TinyBertForSequenceClassification.from_pretrained(args.student_model,
                                                                                       num_labels=num_labels)

                        model_save = get_peft_model(model_save, peft_config)
                        model_save.load_state_dict(student_model.state_dict())
                        model_to_save = model_save.merge_and_unload()
            ...
            tr_att_loss_list.append(tr_att_loss / nb_tr_steps)
            tr_rep_loss_list.append(tr_rep_loss / nb_tr_steps)
            tr_cls_loss_list.append(tr_cls_loss / nb_tr_steps)
        # Create an array for the x-axis (e.g., epochs or iterations)
        epochs = range(len(tr_loss_list))

        # Plotting the lines
        fig = plt.figure(figsize=(10, 6))
        plt.plot(epochs, tr_loss_list, label='Total Loss', color='blue', marker='o')
        plt.plot(epochs, tr_att_loss_list, label='Attention Loss', color='red', marker='x')
        plt.plot(epochs, tr_rep_loss_list, label='Representation Loss', color='green', marker='^')
        plt.plot(epochs, tr_cls_loss_list, label='Classification Loss', color='purple', marker='s')
        ...
        fig.savefig(args.output_dir + '.png', dpi=fig.dpi)
